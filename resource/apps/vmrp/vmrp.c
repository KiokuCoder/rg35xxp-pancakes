...
uc_engine *initVmrp() {
    ...
    // unicorn 在 64 位主机上默认申请 1GiB 的 TCG 翻译缓冲区，掌机上内存不够
    // 会 mmap 失败并直接打印 "Could not allocate dynamic translator buffer" 后
    // exit(1)。mrp 的代码段只有 1MiB，16MiB 的缓冲区完全够用。
    err = uc_ctl_set_tcg_buffer_size(uc, 16 * 1024 * 1024);
    if (err) {
        printf("Failed on uc_ctl_set_tcg_buffer_size() with error returned: %u (%s)\n", err, uc_strerror(err));
        goto end;
    }
